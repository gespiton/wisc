...
if (parsing_step == DEFINE) {
    if (cvar != NULL) {
        error(VAR_ALREADY_EXIST, merge(3, "Variable '", variable->name, "' already exist"), variable->location);
        note(merge(3, "Variable '", variable->name, "' was define here"), cvar->location);
        return 0;
    } else {
